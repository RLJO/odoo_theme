odoo.define('theme_alan.s_product_var_slider_options',function(require){
'use strict';
...
ajax.loadXML('/theme_alan/static/src/xml/product_variant_slider_popup.xml', core.qweb);
...
options.registry['product_variant_slider_actions'] = options.Class.extend({
    ...
    product_variant_slider_configure: function(previewMode, value){
        ...
        if(previewMode === false || previewMode === "click"){
            self.$modal = $(qweb.render("theme_alan.p_var_slider_popup_template"));
            ...
            self._rpc({
                ...
                if(data){
                    ...
                    $('#multiselect_two').multiselect();
                }

                $('#pro_all').on('click', function(e){
                  e.preventDefault();
                  $('#multiselect_two option').prop('selected', true).trigger('chosen:updated');
                });

                $('#pro_reset').on('click', function(e){
                  e.preventDefault();
                  $("#multiselect_two option:selected").removeAttr("selected").trigger('chosen:updated');
                });
            ...
            self.$modal.on('change', self.$modal.find("select[name='slider_type']"), function(e){
                ...
                if(self.$modal.find("select[name='slider_type']").val()){
                    ...
                    self.$modal.find("form .p_slider_sample_view img.snip_sample_img").attr("src", "/theme_alan/static/src/img/snippets/product_slider/" + $sel_ele.val() + ".png");
                }
            });
            // sliding time
            ...
            $('#product_variant_slider_modal').on('shown.bs.modal', function (e) {
                ...
                if(sn_layout != '0'){
                    var split_slider_id = sn_layout.split('_')[2];
                    if(split_slider_id == "7" || split_slider_id == "6" || split_slider_id == "5"){
                        $(".prod_count").hide();
                        $("#prod-auto").parent().hide();
                    }
                    self.$modal.find('form .p_slider_sample_view img.snip_sample_img').attr('src', '/theme_alan/static/src/img/snippets/product_slider/' + sn_layout + '.png');
                }
                else{
                    self.$modal.find("form select[name='slider_type']").val("slider_layout_1");
                }
                if(product_list != '')
                {
                    $('#multiselect_two').val(product_list.split(','));
                    $("#multiselect_two").multiselect("refresh");
                }
            });
        }
        return self;
    ...
    cleanForSave: function () {
        this.$target.empty();
        $('.as_product_variant_slider').empty();
